...
$stmt = $pdo->prepare(
    'SELECT mp.*, cu.full_name AS client_name, cu.email AS client_email, c.contact_number AS client_contact,
            lu.full_name AS lawyer_name, l.gcash_account_name AS lawyer_gcash_account_name, l.gcash_number AS lawyer_gcash_number,
  ...
  <section class="card payment-hero-card payment-admin-hero">
    <div class="payment-admin-hero__content">
      <div class="payment-hero-copy">
        ...
        <p class="muted">Inspect the proof, confirm the lawyer payment details, and record a clear decision for the client.</p>
      </div>
      <div class="payment-progress" aria-label="Payment review stages">
        ...
        <span class="payment-progress-pill <?= $status === 'pending' ? 'is-active' : 'is-done' ?>"><strong>2</strong> Lawyer/Admin review</span>
    ...
    <div class="payment-main">
      <section class="card payment-page-card payment-admin-overview">
        ...
        <div class="payment-admin-metric-grid">
          ...
          <article class="payment-admin-metric-card">
            <span>Lawyer</span>
            <strong><?= !empty($payment['lawyer_name']) ? lex_e((string) $payment['lawyer_name']) : 'Unassigned' ?></strong>
            <small><?= !empty($payment['lawyer_gcash_number']) ? lex_e((string) $payment['lawyer_gcash_number']) : 'No direct QR account' ?></small>
          </article>
          <article class="payment-admin-metric-card">
            <span>Submitted</span>
      ...
      <section class="payment-review-grid payment-admin-review-grid">
        <article class="payment-panel payment-admin-panel">
          ...
          <dl class="payment-account-list payment-admin-details-list">
            ...
            <div><dt>Lawyer GCash</dt><dd><?= !empty($payment['lawyer_gcash_account_name']) ? lex_e((string) $payment['lawyer_gcash_account_name']) : 'Not provided' ?></dd></div>
            <div><dt>GCash number</dt><dd><?= !empty($payment['lawyer_gcash_number']) ? lex_e((string) $payment['lawyer_gcash_number']) : 'Not provided' ?></dd></div>
