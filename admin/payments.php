...
$lawyerEarnings = lex_recent(
    'SELECT l.id, u.full_name AS lawyer_name,
            COUNT(mp.id) AS payment_count,
            SUM(CASE WHEN mp.status = "verified" THEN mp.amount ELSE 0 END) AS verified_total,
            SUM(CASE WHEN mp.status = "pending" THEN 1 ELSE 0 END) AS pending_count
     FROM manual_payments mp
     JOIN lawyers l ON l.id = mp.lawyer_id
     JOIN users u ON u.id = l.user_id
     GROUP BY l.id, u.full_name
     ORDER BY verified_total DESC, payment_count DESC
     LIMIT 8'
);

$query = 'SELECT mp.id, mp.payment_for, mp.amount, mp.reference_number, mp.status, mp.created_at, mp.reviewed_at,
                 cu.full_name AS client_name, cu.email AS client_email, cu.avatar_stored_name AS client_avatar_stored_name,
                 lu.full_name AS lawyer_name, ru.full_name AS reviewed_by_name
          FROM manual_payments mp
...
if ($search !== '') {
    $where[] = '(cu.full_name LIKE :search_name OR cu.email LIKE :search_email OR lu.full_name LIKE :search_lawyer OR mp.payment_for LIKE :search_payment_for OR mp.reference_number LIKE :search_reference)';
    $searchValue = '%' . $search . '%';
    $params['search_name'] = $searchValue;
    $params['search_email'] = $searchValue;
    $params['search_lawyer'] = $searchValue;
...
<section class="card payment-page-card admin-payments-review-card">
  <div class="card-head admin-payments-review-head">
    <div>
      <h2>Reported Lawyer Earnings</h2>
      ...
      <tbody>
        <?php if ($lawyerEarnings): ?>
          <?php foreach ($lawyerEarnings as $earning): ?>
            <tr>
              <td data-label="Lawyer"><strong><?= lex_e((string) $earning['lawyer_name']) ?></strong></td>
              <td data-label="Verified earnings">PHP <?= lex_e(number_format((float) ($earning['verified_total'] ?? 0), 2)) ?></td>
              <td data-label="Total records"><?= (int) ($earning['payment_count'] ?? 0) ?></td>
              <td data-label="Pending review"><?= (int) ($earning['pending_count'] ?? 0) ?></td>
            </tr>
          <?php endforeach; ?>
        <?php else: ?>
          <tr><td colspan="4" class="muted">No lawyer earnings have been recorded yet.</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
  ...
  <div class="table-wrap">
    <table class="data-table payment-history-table">
      ...
      <tbody>
        ...
        <?php else: ?>
          <tr>
            <td colspan="8" class="muted">No payments matched this filter.</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
