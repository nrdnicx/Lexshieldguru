...
$gcashQrUrl = $gcashQrStoredName !== '' ? lex_app_url('payment_qr_image.php?lawyer_id=' . $lawyerId) : '';
            ...
            if ($stmt->fetchColumn()) {
                ...
                if ($error === '') {
                    ...
                    $profileChanged = $fullName !== (string) ($profile['full_name'] ?? '')
                        ...
                        || (bool) $newAvatar;
                    try {
                        $newPaymentQr = lex_store_payment_qr($_FILES['gcash_qr_file'] ?? []);
                    } catch (RuntimeException $exception) {
                        $error = $exception->getMessage();
                    }
                }
                ...
                if ($error === '') {
                    $profileChanged = $profileChanged || (bool) $newPaymentQr;
                    ...
                    if ($newPaymentQr) {
                        $lawyerUpdate .= ', gcash_qr_stored_name = :gcash_qr_stored_name';
                        $lawyerParams['gcash_qr_stored_name'] = $newPaymentQr['stored_name'];
                    }
                    $lawyerUpdate .= ' WHERE id = :id';
                    $pdo->prepare($lawyerUpdate)->execute($lawyerParams);
                    ...
                    if ($newPaymentQr && $gcashQrStoredName !== '' && $gcashQrStoredName !== $newPaymentQr['stored_name']) {
                        $oldQrPath = lex_payment_qr_path($gcashQrStoredName);
                    ...
                    if ($newPaymentQr && is_file($newPaymentQr['path'])) {
                        @unlink($newPaymentQr['path']);
                    }
...
<section class="law-profile-shell law-profile-shell--lawyer">
  <div class="law-profile-grid">
    ...
    <article class="law-profile-info-card">
      ...
      <div class="law-profile-detail-list">
        ...
        <div class="law-profile-detail-item">
          <span class="law-profile-detail-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M4.75 6h14.5A1.75 1.75 0 0 1 21 7.75v8.5A1.75 1.75 0 0 1 19.25 18H4.75A1.75 1.75 0 0 1 3 16.25v-8.5A1.75 1.75 0 0 1 4.75 6Zm0 2.5v1h14.5v-1Zm9.75 5.25a.75.75 0 0 0 0 1.5h2.5a.75.75 0 0 0 0-1.5Z" fill="currentColor"/></svg></span>
          <div><span>GCash Account</span><strong><?= lex_e($gcashAccountName !== '' ? $gcashAccountName : 'Not provided') ?></strong></div>
        </div>
        <div class="law-profile-detail-item">
          <span class="law-profile-detail-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M6.75 3.5h10.5A1.75 1.75 0 0 1 19 5.25v13.5A1.75 1.75 0 0 1 17.25 20H6.75A1.75 1.75 0 0 1 5 18.75V5.25A1.75 1.75 0 0 1 6.75 3.5Zm2 4.25a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5Zm0 4a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5Z" fill="currentColor"/></svg></span>
          <div><span>GCash Number</span><strong><?= lex_e($gcashNumber !== '' ? $gcashNumber : 'Not provided') ?></strong></div>
        </div>
        ...
        <div class="law-profile-detail-item law-profile-detail-item--wide">
          <span class="law-profile-detail-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M5.25 4h13.5A1.25 1.25 0 0 1 20 5.25v13.5A1.25 1.25 0 0 1 18.75 20H5.25A1.25 1.25 0 0 1 4 18.75V5.25A1.25 1.25 0 0 1 5.25 4Zm1.25 2.5v4h4v-4h-4Zm7 0v4h4v-4h-4Zm-7 7v4h4v-4h-4Zm7 0v4h4v-4h-4Z" fill="currentColor"/></svg></span>
          <div><span>Payment QR</span><strong><?= $gcashQrUrl !== '' ? 'Uploaded' : 'Not uploaded' ?></strong></div>
        </div>
      </div>
    </article>
  </div>
  ...
  <article class="law-profile-actions-card">
    ...
    <div class="law-profile-action-grid">
      ...
      <a class="law-profile-action" href="<?= lex_e(lex_app_url('lawyer/payments.php')) ?>"><span class="tone-green"><svg viewBox="0 0 24 24"><path d="M4.75 6h14.5A1.75 1.75 0 0 1 21 7.75v8.5A1.75 1.75 0 0 1 19.25 18H4.75A1.75 1.75 0 0 1 3 16.25v-8.5A1.75 1.75 0 0 1 4.75 6Zm0 2.5v1h14.5v-1Zm9.75 5.25a.75.75 0 0 0 0 1.5h2.5a.75.75 0 0 0 0-1.5Z" fill="currentColor"/></svg></span><div><strong>Payments</strong><small>Verify client proofs</small></div><b>&rsaquo;</b></a>
  ...
  <div class="modal-card wide profile-edit-modal" role="dialog" aria-modal="true" aria-labelledby="profileEditTitle">
    ...
    <div class="modal-body">
      ...
      <form method="post" class="form-grid" enctype="multipart/form-data">
        ...
        <label>GCash number
          <input type="text" name="gcash_number" inputmode="tel" placeholder="09xxxxxxxxx" value="<?= lex_e($gcashNumber) ?>">
        </label>
        <label class="full">GCash QR code
          <div class="profile-upload-row">
            <div class="profile-upload-thumb">
              <?php if ($gcashQrUrl !== ''): ?>
                <img src="<?= lex_e($gcashQrUrl) ?>" alt="Current GCash QR for <?= lex_e($fullName) ?>">
              <?php else: ?>
                <span>QR</span>
              <?php endif; ?>
            </div>
            <div class="profile-upload-field">
              <input type="file" name="gcash_qr_file" accept="image/png,image/jpeg,image/gif,image/webp">
              <p class="modal-note">Clients will scan this QR and pay directly to your account. JPG, PNG, GIF, or WEBP. Max 5 MB.</p>
            </div>
          </div>
        </label>
