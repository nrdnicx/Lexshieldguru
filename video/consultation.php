...
if ($provider === 'jaas') {
    if (lex_video_token_secret() === '') {
        ...
        $localJwt = function_exists('openssl_sign') ? lex_video_jaas_jwt($meeting, $user) : null;
        if ($localJwt !== null) {
            $jaasJwt = $localJwt;
            $jaasError = '';
        } elseif (is_array($jaasStatus) && !$jaasStatus['openssl_available']) {
            $jaasError .= ' PHP OpenSSL is unavailable, so local token fallback also cannot run.';
        }
    }
}
